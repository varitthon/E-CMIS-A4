  ...
  function renderStaff(){
    ...
    if(searchInput){
      ...
      searchInput.addEventListener('input',()=>{
        ...
        searchTimer=setTimeout(()=>{
          ...
          let html='';
          if(caseHits.length){html+=`<div class="ws-search-group">คดี (${caseHits.length})</div>`+caseHits.slice(0,6).map(st=>{const c=st.caseData||st;return`<button type="button" class="ws-search-item" data-search-case="${c.id}"><span>${escapeHtml(c.subject||c.id)}</span><small>เลขรับเรื่อง ${escapeHtml(registryReceiptNumber(c)||c.id)} · เลขรับบริการ ${escapeHtml(c.trackingYear)} · PIN ${escapeHtml(c.trackingCode)} · ${escapeHtml(c.channel)}</small></button>`}).join('')}
    ...
    function adminScreeningBlock(state){
      ...
      const decidedSummary=decided==='duplicate'
        ?`<div class="screening-summary"><span class="ws-status danger">เรื่องซ้ำ — รวมกับเรื่องเดิม ${escapeHtml(d.screening.linkedCaseId||'')}</span><p>ตัดสินใจโดย ${escapeHtml(d.screening.decidedBy||'—')} · ${escapeHtml(d.screening.decidedAt||'—')}</p><p>หมายเหตุ: ${escapeHtml(d.screening.note||'—')}</p></div>`
        :decided==='new'
