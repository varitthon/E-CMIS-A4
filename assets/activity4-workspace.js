(() => {
  ...
  function receiptPaper(c,qr='',opts={}){
    ...
    return `<div class="document-page-set" id="activePaper">
      <article class="a4-paper template-receipt">
        ...
        ${isDuplicate?`<p class="receipt-duplicate-note"><strong>เรื่องซ้ำ — ใช้เลขติดตามเดิม</strong> เรื่องนี้เป็นเรื่องซ้ำของเรื่องเดิม (${escapeHtml(opts.duplicateOf||'')}) และไม่มีการออกเลขติดตามใหม่ กรุณาใช้เลขรับบริการและ PIN ข้างต้น (เลขเดิม) เพื่อติดตามสถานะเรื่องร้องเรียน</p>`:''}
  ...
  function duplicateReceiptPaper(newCase,originalCase,qr){
    return receiptPaper(newCase,qr,{duplicateOf:originalCase?.id||'',originalTracking:originalCase?.trackingYear||'',originalPin:originalCase?.trackingCode||''});
  }
  ...
  function renderStaff(){
    ...
    const duplicateObserver=new MutationObserver(()=>{
      ...
      if(window.Swal&&$('#adminNote')){sessionStorage.setItem(warningKey,'shown');setTimeout(()=>Swal.fire({icon:'warning',title:'ระบบพบเรื่องที่อาจซ้ำ',html:`<div class="duplicate-alert-card"><span>ความใกล้เคียง</span><strong>${escapeHtml(result.dataset.score)}%</strong><p>พบความเชื่อมโยงกับ <b>${escapeHtml(result.dataset.caseId)}</b></p><p>โปรดบันทึกความเห็นประกอบก่อนมอบหมายงาน</p></div>`,width:680,confirmButtonText:'รับทราบ',confirmButtonColor:'#082b50'}),0);}
    });
    ...
    function screeningAnalysisCard(c){
      ...
      return `<div class="duplicate-auto-result ${analysis.level==='high'?'duplicate-critical':analysis.level==='review'?'duplicate-review':''}" data-level="${analysis.level}" data-case-id="${escapeHtml(analysis.caseId)}" data-score="${analysis.score}"><div class="duplicate-score"><span>ความใกล้เคียง</span><strong>${analysis.score}<small>%</small></strong><b>${levelLabel}</b></div><div class="duplicate-result-detail"><span>ระบบตรวจสอบอัตโนมัติ (คำแนะนำ)</span><strong>${analysis.level==='high'?'พบความเสี่ยงเรื่องซ้ำระดับสูง':analysis.level==='review'?'พบเรื่องที่ควรตรวจสอบเพิ่มเติม':'ยังไม่พบเรื่องที่มีความใกล้เคียงอย่างมีนัยสำคัญ'}</strong>${analysis.caseId?`<p>เชื่อมโยงกับเรื่องเดิม <b>${escapeHtml(analysis.caseId)}</b></p>`:''}<div class="duplicate-signal-list">${analysis.signals.map(signal=>`<small>${escapeHtml(signal)}</small>`).join('')}</div></div></div>`;
    }
    function screeningDecisionForm(state,{withAdditionalInfo=false}={}){
      ...
      <div id="screeningDuplicateBox" class="ws-hidden">
        <div class="ws-field"><label>เลือกเรื่องเดิม *</label><select id="screeningLinkedCase"><option value="">— เลือกเรื่องเดิม —</option>${candidates.map(cd=>`<option value="${escapeHtml(cd.id)}" ${cd.id===analysis.caseId?'selected':''}>${escapeHtml(cd.id)} · ${escapeHtml(cd.complainant)} · ${escapeHtml(cd.subject)}</option>`).join('')}</select></div>
    ...
    function adminScreeningBlock(state){
      const d=state.documentData,c=state.caseData;
      if(isOfficerScreenedChannel(c.channel)){
        return `${screeningAnalysisCard(c)}<p class="duplicate-policy-note">ผลวิเคราะห์เป็นข้อมูลประกอบการพิจารณา — ช่องทาง Walk-in/1206 เจ้าหน้าที่รับเรื่องเป็นผู้ตัดสินใจกลั่นกรองเรื่องซ้ำ ระบบไม่ตัดสินใจแทน</p><div class="ws-field screening-admin-opinion"><label>ความเห็นธุรการ (ส่งต่อให้เจ้าหน้าที่รับเรื่อง)</label><textarea id="adminNote" placeholder="บันทึกความเห็นหรือข้อสังเกตเพื่อส่งต่อให้เจ้าหน้าที่รับเรื่อง">${escapeHtml(d.adminNote)}</textarea></div>${d.mergedDuplicates?.length?`<div class="screening-merged-list"><h4>เรื่องซ้ำที่รวมเข้ามา</h4><ul>${d.mergedDuplicates.map(m=>`<li>${escapeHtml(m.caseId)} · ${escapeHtml(m.at)}${m.note?` — ${escapeHtml(m.note)}`:''}</li>`).join('')}</ul></div>`:''}`;
      }
      if(d.screening?.decision==='duplicate'){
        return `<div class="screening-summary"><span class="ws-status danger">เรื่องซ้ำ — รวมกับเรื่องเดิม ${escapeHtml(d.screening.linkedCaseId||'')}</span><p>ตัดสินใจโดย ${escapeHtml(d.screening.decidedBy||'—')} · ${escapeHtml(d.screening.decidedAt||'—')}</p><p>หมายเหตุ: ${escapeHtml(d.screening.note||'—')}</p><div class="screening-actions"><button type="button" class="ws-button secondary" data-action="print-duplicate-receipt">พิมพ์ใบแจ้ง (เลขเดิม)</button><button type="button" class="ws-button ghost" data-action="screening-undo">ยกเลิกการตัดสินใจ</button></div></div>`;
      }
      if(d.screening?.decision==='new'){
        return `<div class="screening-summary"><span class="ws-status success">เรื่องใหม่ — ลงรับและมอบหมายต่อ</span><p>ตัดสินใจโดย ${escapeHtml(d.screening.decidedBy||'—')} · ${escapeHtml(d.screening.decidedAt||'—')}</p><div class="screening-actions"><button type="button" class="ws-button ghost" data-action="screening-undo">เปลี่ยนการตัดสินใจ</button></div></div>`;
      }
      return `${screeningAnalysisCard(c)}${screeningDecisionForm(state)}<div class="screening-actions"><button type="button" class="ws-button primary" data-action="screening-confirm">ยืนยันการกลั่นกรอง</button></div><p class="duplicate-policy-note">ผลวิเคราะห์เป็นข้อมูลประกอบการพิจารณา — เจ้าหน้าที่เป็นผู้ตัดสินใจ ระบบไม่ตัดสินใจแทน ธุรการไม่มีสิทธิยุติเรื่องจากผลตรวจอัตโนมัติเพียงอย่างเดียว</p>`;
    }
    ...
    async function handleAction(action,state){
      ...
      if(action==='screening-confirm'){
        ...
        if(screeningChoice==='duplicate'){
          ...
          const confirmed=await confirmDo('ยืนยันเรื่องซ้ำ — รวมกับเรื่องเดิม',`เรื่องนี้จะไม่มีการออกเลขใหม่ และจะรวมกับเรื่องเดิม ${linkedCaseId}${linkedTracking?` (เลขติดตามเดิม ${linkedTracking})`:''}${additionalInfo?`\nข้อมูลเพิ่มเติมจะส่งเข้าเรื่องเดิม: ${additionalInfo}`:''}`,'ยืนยันเรื่องซ้ำ');
          if(!confirmed.isConfirmed)return;
          ...
          w.owner=activeRole;w.stage='duplicate-closed';w.status=`เรื่องซ้ำ — รวมกับเรื่องเดิม ${linkedCaseId}`;w.complete=true;
          add(`ตัดสินใจกลั่นกรอง: เรื่องซ้ำ — รวมกับเรื่องเดิม ${linkedCaseId}${linkedTracking?` (เลขติดตามเดิม ${linkedTracking})`:''}${screeningNote?` — ${screeningNote}`:''}`);
          if(originalState){
            originalState.decisionHistory=originalState.decisionHistory||[];
            originalState.decisionHistory.push({text:`มีเรื่องซ้ำรวมเข้ามา: ${state.caseData.id} (${state.caseData.received})${additionalInfo?` — ข้อมูลเพิ่มเติม: ${additionalInfo}`:''}`,time:now()});
          ...
          notify('success','บันทึกเรื่องซ้ำแล้ว',`เรื่องถูกรวมกับเรื่องเดิม ${linkedCaseId} — ไม่มีการออกเลขใหม่ และเรื่องเดิมได้รับแจ้งแล้ว`);
      ...
      if(action==='screening-undo'){
        ...
        if(w.stage==='duplicate-closed'){w.owner='officer';w.stage='officer';w.status='รอเจ้าหน้าที่พิจารณา';w.complete=false}
      ...
      if(action==='officer-send'){if(!['officer','admin'].includes(w.owner)&&!w.complete)return notify('warning','ข้อมูลถูกล็อค','เรื่องนี้อยู่ระหว่างการพิจารณาของผู้มีอำนาจ');
        if(isOfficerScreenedChannel(state.caseData.channel)&&d.screening?.decision!=='new')return notify('warning','ยังไม่ได้กลั่นกรองเรื่องซ้ำ','ต้องตัดสินใจกลั่นกรอง (เรื่องใหม่) ก่อนบันทึกและส่งพิจารณา');
